...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            <a class="btn btn-primary" href="{{ route('grupo.create') }}"> Registrar Grupo</a>
        </div>
        <!-- /.card-header -->
        <div class="card-body">
            ...
            <table id="Tablagrupos" class="table table-bordered table-hover text-center">
                <thead>
                    <tr>
                        ...
                        <th scope="col"> N° Alum registrados</th>
                ...
                <tbody>
                    @foreach ($grupos as $grup)
                        ...
                        <tr>
                            <td> {{ $grup->id }}</td>
                            <td> {{ $grup->cod_group }}</td>
                            <td> {{ $grup->name_group }}</td>
                            <td> {{ $grup->name_department . ' / ' . $grup->name_municipios }} </td>
                            <td> {{ $grup->grade_group }}</td>
                            <td>


                                <a class="btn btn-default " style="width: 43%; "
                                    href="{{ route('grupo.estudiante', $grup) }}">
                                    <i class=" fas fa-solid fa-eye"></i>

                                    @if ($grup->prueba($grup->id) != null)
                                        {{ $grup->prueba($grup->id) }}
                                    @else
                                        0
                                    @endif
                            ...
                            <td>
                                {{-- <form  action="{{ route('grupo.destroy', $grup) }}" 
                                        method="POST"> --}}
                                {{-- podria usar la calse btn-sm para hacer los botones mas pequeños --}}
                                <a class="btn btn-info btn-sm" href="{{ route('grupo.edit', $grup) }}">
...
@section('js')
    ...
    @if (session('info'))
        <script>
            $(document).ready(function() {
                toastr["info"]("Muy bien, el grupo se ha actualizado con exito", "Mensaje de alerta");
                toastr.options = {
                    //primeras opciones
    ...
    @else
        @if (session('info2'))
            <script>
                $(document).ready(function() {
                    toastr["success"]("Muy bien, el grupo se ha agregado con exito.", "Mensaje de alerta");
                    toastr.options = {
                        //primeras opciones
                        "closeButton": false, //boton cerrar
                        "debug": false,
