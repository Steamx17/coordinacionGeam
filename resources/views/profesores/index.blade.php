...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            <a class="btn btn-primary" href="{{ route('docente.create') }}"> Registrar un Docente</a>
        </div>
        <!-- /.card-header -->
        <div class="card-body">
            <!-- <table id="example1" class="table table-bordered table-striped">-->

            <table id="Tablagrupos" class="table table-bordered table-hover text-center">
                <thead>

                    <tr>
                        <th spcoe="col">#</th>
                        <th spcoe="col">Nombres</th>
                        <th spcoe="col">Apellidos</th>
                        <th spcoe="col">Asignatura</th>
                        <th spcoe="col">Status</th>
                        <th spcoe="col">Acciones</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($docentesx as $docent)

                        <tr>
                            <td> {{ $docent->id }}</td>
                            <td> {{ $docent->names_teacher }}</td>
                            <td> {{ $docent->surnames_teacher }}</td>
                            <td> {{ $docent->subjects->name_subjects }} </td>
                            <td>
                                @if ($docent->status == 'INACTIVE')
                            ...
                            <td>
                                <form action="{{ route('grupo.destroy', $docent) }}" method="POST">
                                    {{-- podria usar la calse btn-sm para hacer los botones mas pequeños --}}
                                    <a class="btn btn-info btn-sm" href="{{ route('docente.edit', $docent) }}">
...
@section('js')
    ...
    @if (session('info'))
        <script>
            $(document).ready(function() {
                toastr["info"]("Muy bien, el docente se ha actualizado con exito", "Mensaje de alerta");
                toastr.options = {
                    //primeras opciones
    ...
    @else
        @if (session('info2'))
            <script>
                $(document).ready(function() {
                    toastr["success"]("Muy bien, el docente se ha agregado con exito.", "Mensaje de alerta");
                    toastr.options = {
                        //primeras opciones
                        "closeButton": false, //boton cerrar
                        "debug": false,
...
<script>
    ...
    function EliminarConfirmar(linkURL) {
        swal({
            title: "Esta seguro que quiere eliminar este docente ?",
            ...
            if (result.value) {
               ...
               e.submit();
                ...
                swal("Cancelado", "El docente aun continua en el sistema.", "error");
            }
        });
    }
</script>
